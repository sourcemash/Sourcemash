...
feeds.feedList.create(this.newAttributes(),{wait:true,success:this.clear_input});feeds.feedList.fetch(); return false;},clear_input:function(){$('#url').val('');},addOne:function(feed){var view=new feeds.FeedView({model:feed});$('#feed-list').append(view.render().el);},addAll:function(){$('#feed-list').html(''); feeds.feedList.each(this.addOne,this);},newAttributes:function(){return{url:this.input.val(),csrf_token:this.csrf.val()}}});
