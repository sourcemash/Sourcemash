...
feeds.feedList.on('sync',this.addAll,this);feeds.feedList.fetch();},events:{'keypress #url':'createFeedOnEnter','click button[type="submit"]':'createFeedOnClick'},createFeedOnClick:function(e){feeds.feedList.create(this.newAttributes(),{wait:true,success:this.clearInput});feeds.feedList.fetch(); return false;},createFeedOnEnter:function(e){if(e.which!==13||!this.input.val().trim()){ return;}
feeds.feedList.create(this.newAttributes(),{wait:true,success:this.clearInput});feeds.feedList.fetch(); return false;},clearInput:function(){$('#url').val('');},addOne:function(feed){var view=new feeds.FeedView({model:feed});$('#feed-list').append(view.render().el);},addAll:function(){$('#feed-list').html(''); feeds.feedList.each(this.addOne,this);},newAttributes:function(){return{url:this.input.val(),csrf_token:this.csrf.val()}}});
